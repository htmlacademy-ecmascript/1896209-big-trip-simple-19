...
function createFormTemplate(state, offersByType, destinations) {
  ...
  let offersTemplate = '';
  if (offers.length > 0) {
    const offersTemplateList = offers.map((offer) => renderOfferForType(offer, state.offers.some((pointOffer) => pointOffer.id === offer.id), state.isDisabled))
      .join('\n');
    offersTemplate = createOffersContainer(offersTemplateList);
  }

  const eventTypeTemplate = offersByType.map((offer) => getEventTypeItem(offer.type)).join('\n');
